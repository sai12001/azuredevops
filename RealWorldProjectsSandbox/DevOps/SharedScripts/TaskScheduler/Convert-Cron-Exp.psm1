...
function ConvertFrom-PodeCronExpression
{
    ...
    if (!([string]::IsNullOrEmpty($predef[$Expression]))) {
...
function datetime-construct {
    ...
    $dateconstruct=$dateprefix + "/" + $hour + "/" +$minute + "/00"
    $DateTimeObject = [datetime]::ParseExact($dateconstruct,"yyyy/MM/dd/HH/mm/ss",$null)
